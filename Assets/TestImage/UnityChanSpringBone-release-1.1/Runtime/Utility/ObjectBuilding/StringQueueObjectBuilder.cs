        ...
        public static class ObjectBuilder
        {
            ...
            public static Transform DequeueTransform(this Queue<string> queue, GameObject gameObject)
            {
                ...
                if (parentName.Length > 0)
                {
                    ...
                    // [수정] FindObjectsOfType -> FindObjectsByType (SortMode.None으로 성능 최적화)
                    newParent = Object.FindObjectsByType<Transform>(FindObjectsSortMode.None)
                        .Where(item => item.name == parentName && !children.Contains(item))
                        .FirstOrDefault();
            ...
            {
                // [수정] FindObjectsOfType -> FindObjectsByType 사용
                // 단일 객체를 찾는 로직이므로 정렬이 필요 없어 FindObjectsSortMode.None 적용
